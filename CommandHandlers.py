...
def send_server_message(client, message):
    ...
    client.socket.send(json.dumps({'type': 'server_message', 'message': message}).encode())  
...
def handle_join(client, message, channels):
    ...
    broadcast(client.socket, {'type': 'server_message', 'message': f"SERVER: {client.nickname} has joined the channel."}, channel) 
...
def handle_leave(client, message):
    ...
    if client.channel:  # Check if the client is in a channel
        ...
        broadcast(client.socket, {'type': 'server_message', 'message': f"SERVER: {client.nickname} has left the channel."}, client.channel)  

        client.channel.clients.remove(client)  # Remove the client from the channel's client list
...
def handle_nick(client, message):
    ...
    if new_nickname:  # If a new nickname is provided
        ...
        if client.channel:  # If the client is in a channel
            ...
            broadcast(client.socket, {'type': 'server_message', 'message': f"SERVER: {old_nickname} is now known as {client.nickname}."}, client.channel)  
...
def handle_msg(client, message):
    ...
    try:
        target_nickname = message['target_nickname']  # Get the target nickname from the message
        ...
        if target_client:  # If the target client is found
            ...
            target_client.socket.send(json.dumps({'type': 'private_message', 'sender': client.nickname, 'message': private_message}).encode())  
            client.socket.send(json.dumps({'type': 'private_message', 'receiver': target_nickname, 'message': private_message}).encode())  
        else:  # If the target client is not found
            send_server_message(client, f"SERVER: User {target_nickname} not found.")  # Inform the sender
    except IndexError:  # Handle potential errors in message formatting
        send_server_message(client, "Invalid /msg command format. Usage: /msg <nickname> <message>")  # Inform the sender
