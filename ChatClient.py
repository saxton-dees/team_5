        ...
        try:
            ...
            if message['type'] == 'server_message':  # Server messages
                ...
                if 'sender' in message:
                    print(f"PRIVATE MESSAGE from {message['sender']}: {message['message']}")
                else:
                    print(f"PRIVATE MESSAGE to {message['receiver']}: {message['message']}")
...
while True:
    ...
    try:
        if message.startswith("/connect"):  # Connect to the server
            parts = message.split()
            send_message('connect', server_name=parts[1], port=int(parts[2]) if len(parts) > 2 else None)
            ...
            parts = message.split()
            send_message('msg', target_nickname=parts[1], private_message=" ".join(parts[2:]))
